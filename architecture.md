...
\sum_{i=1}^{N}
\left|
\operatorname{clip}\!\left(f_{\theta}(\mathbf{z}_j,\mathbf{x}_i), \tau_{\min}, \tau_{\max}\right)
-
\operatorname{clip}\!\left(s_i, \tau_{\min}, \tau_{\max}\right)
\right|
\;+\;
...
- $N$ 是当前样本使用的 SDF 采样点数；
- `clip` 对应代码里的 `torch.clamp`，用于把 GT 和预测值同时截断到 $[\tau_{\min}, \tau_{\max}]$。

当前实现里的主重建项是 **L1 SDF 重建损失**，不是 MSE。
